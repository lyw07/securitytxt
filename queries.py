...
def query_contact_categories(args):
    ...
    # draw the images to compare with the original paper's results
    old_results = [41, 41, 8, 1, 9]
    new_results = [67, 12, 20, 0.7, 0.3]
    x = [0, 1]
    xticks = ["2021-05", "2022-03"]
    combined = list(zip(old_results, new_results))
    y1 = list(combined[0])
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111)
    plt.plot(x, y1, color="royalblue", marker=".", label="Email Only")
    for i, v in enumerate(y1):
        ax.text(i, v + 0.4, "%.1f" % v, ha="center")
    plt.plot(x, y2, color="darkorange", marker="^", label="URL Only")
    plt.plot(x, y3, color="g", marker="*", label="Email and URL")
    plt.plot(x, y4, color="r", marker="d", label="Telephone Number")
    plt.plot(x, y5, color="c", marker="o", label="Others")
    plt.xticks(x, xticks)
    plt.xlabel("Date")
